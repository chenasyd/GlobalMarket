package com.upgrade.globalmarket.internal.data;

import com.upgrade.globalmarket.api.handler.sort.SortHandler;
import com.upgrade.globalmarket.internal.enums.ActionType;
import com.upgrade.globalmarket.internal.plugin.GlobalMarket;
import com.upgrade.globalmarket.internal.command.virtual.VirtualMarketCommand;
...
public class DataContainer {
    ...
    public static void loadData() {
        GlobalMarket.getInstance().saveResource("types.yml", "types.yml", false, (file) -> {
        ...
        GlobalMarket.getInstance().saveResource("sorts.yml", "sorts.yml", false, (file) -> {
        ...
        // Save all data
        if (!MARKET_DATA.isEmpty() && GlobalMarket.getStorageHandler() != null) {
            GlobalMarket.getStorageHandler().saveAll();
        }
        // Load market data
        File file = new File(GlobalMarket.getInstance().getDataFolder(), "market");
        if (!file.exists()) {
            file.mkdir();
            ...
            GlobalMarket.getInstance().saveResource(sourceFile, "market/example.yml");
        }
        MARKET_DATA.clear();
    ...
    private static void loadActions() {
        GlobalMarket.getInstance().saveResource("action/market.yml", "action/market.yml", false, (file) -> {
            ...
            data.getKeys(false).forEach((key) -> {
                try {
                    ...
                    GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to load market action: " + key);
                }
            });
        });
    }
    ...
    private static void registerVirtualMarketCommands() {
        try {
            if (Bukkit.getPluginManager() instanceof SimplePluginManager) {
                ...
                new ArrayList<>(commandMap.getCommands()).forEach((s) -> {
                    ...
                    if (s.unregister(commandMap)) {
                        ...
                        knownCommands.remove((GlobalMarket.getInstance().getDescription().getName() + ":" + s.getLabel()).toLowerCase());
                ...
                DataContainer.MARKET_DATA.forEach((k, v) -> {
                    ...
                    if (commandMap.register(GlobalMarket.getInstance().getDescription().getName(), command)) {
            ...
            GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to register virtual command.");
        }
    }

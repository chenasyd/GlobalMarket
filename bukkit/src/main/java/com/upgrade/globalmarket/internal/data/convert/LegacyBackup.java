package com.upgrade.globalmarket.internal.data.convert;
...
public class LegacyBackup {
    ...
    public static void check() {
        if (GlobalMarket.getInstance().getDataFolder().exists()) {
            File file = new File(GlobalMarket.getInstance().getDataFolder(), "history.yml");
            if (!file.exists()) {
                File backupFolder = new File("./backups");
                backupFolder.mkdir();

                GlobalMarket.getInstance().getConsoleLogger().log(false, I18n.getProperties().getProperty("backup-start"));
                // backups data
                try {
                    String fileName = "ServerMarket-" + System.currentTimeMillis();
                    Files.createDirectories(Paths.get("./backups", fileName));
                    LegacyBackup.copyFile(GlobalMarket.getInstance().getDataFolder(), new File(backupFolder, fileName));
                    GlobalMarket.getInstance().saveResource("history.yml", "history.yml");
                    GlobalMarket.getInstance().getConsoleLogger().log(false,
                            I18n.getProperties().getProperty("backup-completed").replace("%s", fileName));
                } catch (IOException e) {
                    GlobalMarket.getInstance().getLogger().log(Level.SEVERE, e, () -> "Failed to backup data.");
                    Bukkit.getPluginManager().disablePlugin(GlobalMarket.getInstance());
                }
            }
        }
    }

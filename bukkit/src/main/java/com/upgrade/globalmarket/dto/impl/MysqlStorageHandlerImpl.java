package com.upgrade.globalmarket.dto.impl;
...
public class MysqlStorageHandlerImpl extends AbstractStorageHandler {
    ...
    public void initialize() {
        ...
        storageHandler = new MySqlStorageHandler(
                ...
                GlobalMarket.getInstance().getConfig().getString("data-option.url"),
                GlobalMarket.getInstance().getConfig().getString("data-option.user"),
                GlobalMarket.getInstance().getConfig().getString("data-option.password"),
                sqlArray
        );
    ...
    public boolean hasSale(String market, String saleId) {
        ...
        storageHandler.connect((preparedStatement) -> {
            try {
                ...
                GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to check sale item.");
            }
    ...
    public int getSaleCountByPlayer(UUID uuid, String market) {
        ...
        storageHandler.connect((preparedStatement) -> {
            try {
                ...
                GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to get sale count.");
            }
        ...
        DataContainer.MARKET_DATA.forEach((k, v) -> {
            for (Map.Entry<String, SaleCache> entry : this.getSaleItemsByMarket(k).entrySet()) {
                ...
                if (second >= v.getEffectiveTime()) {
                    ...
                    GlobalMarket.getStorageHandler().removeSaleItem(k, entry.getKey()).ifPresent((sale) -> {
                        GlobalMarket.getStorageHandler().addItemToStore(uuid, sale.getSaleItem(), "timeout");
                    });
                }
            }
        });
            ...
            try {
                ...
                if (connection != null) {
                    try {
                        ...
                        GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to rollback data");
                    }
                }
                GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to import data");
            }
        }, "INSERT INTO " + salesTable + " (sale_uuid,market,owner_name,owner_uuid,pay_type,eco_type,price,data,post_time) VALUES (?,?,?,?,?,?,?,?,?);");
    }
        ...
        if (playerCache.isNewData()) {
            storageHandler.connect((preparedStatement) -> {
                try {
                    ...
                    GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to save player data.");
                }
    ...
    public void setLock(UUID uuid, boolean locked) {
        storageHandler.connect((preparedStatement) -> {
            try {
                ...
                GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to set player lock.");
            }
    ...
    public boolean isLocked(UUID uuid) {
        ...
        storageHandler.connect((preparedStatement) -> {
            try {
                ...
                GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to check player lock.");
            }
        ...
        if (forceLoad) {
            ...
            storageHandler.connect((preparedStatement) -> {
                try {
                    ...
                    GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to load player data.");
                }
    ...
    public ItemStack removeStoreItem(UUID uuid, String storeItemId) {
        ...
        try {
            ...
            GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "cannot add item to store: " + uuid.toString());
            return null;
        }
    }
    ...
    public void addOfflineTransaction(OfflineTransactionData data) {
        storageHandler.connect((preparedStatement) -> {
            try {
                ...
                GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to add offline transaction." + builder);
            }
    ...
    public boolean removeOfflineTransaction(String key) {
        ...
        storageHandler.connect((preparedStatement) -> {
            try {
                ...
                GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to remove offline transaction.");
            }
    ...
    public Map<String, OfflineTransactionData> getOfflineTransactionByPlayer(UUID ownerUniqueId) {
        ...
        storageHandler.connect((preparedStatement) -> {
            try {
                ...
                GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "Failed to get offline transactions." + builder);
            }

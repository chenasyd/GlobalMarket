...
import com.upgrade.globalmarket.internal.cache.market.MarketCache;
...
public class YamlStorageHandlerImpl extends AbstractStorageHandler {
    ...
    private void saveResults() {
        ...
        try {
            ...
            GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "cannot save result data");
        }
    }
    ...
    private void saveLogs() {
        synchronized (LOG_SECTION_MAP) {
            File logFolder = new File(GlobalMarket.getInstance().getDataFolder(), "logs");
            logFolder.mkdir();
            ...
            try {
                ...
                GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "cannot save log data: " + format + ".yml");
            }
        }
    }
    ...
    public void initialize() {
        GlobalMarketApi.getPlatformApi().runTaskTimerAsynchronously(GlobalMarket.getInstance(), this::saveLogs, 3, 3);
    }

    @Override
    public void reload() {
        File dataFolder = new File(GlobalMarket.getInstance().getDataFolder(), "data");
        dataFolder.mkdirs();
        // check logs
    ...
    public void load(String market) {
        ...
        File file = new File(GlobalMarket.getInstance().getDataFolder() + "/saleData/", market + ".yml");
    ...
    public void save(String market, Map<String, SaleCache> map) {
        ...
        try {
            ...
            GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "cannot save sale data: " + market);
        }
    }
        ...
        MARKET_STORAGE_DATA_MAP.forEach((k, v) -> {
            ...
            for (Map.Entry<String, SaleCache> entry : v.getSales().entrySet()) {
                ...
                if (second >= marketConfigData.getEffectiveTime() && this.removeSaleItem(k, entry.getValue().getSaleUUID()).isPresent()) {
                    ...
                    GlobalMarket.getStorageHandler().addItemToStore(uuid, entry.getValue().getSaleItem(), "timeout");
                }
            }
        });
    }
    ...
    public void savePlayerData(PlayerCache playerCache, boolean removeCache) {
        File file = new File(GlobalMarket.getInstance().getDataFolder() + "/data/", playerCache.getOwnerUniqueId().toString() + ".yml");
        FileConfiguration data = playerCache.saveToConfiguration();
        try {
            data.save(file);
        } catch (IOException e) {
            GlobalMarket.getInstance().getLogger().log(Level.WARNING, e, () -> "cannot save player data: " + playerCache.getOwnerUniqueId().toString());
        }
        if (removeCache) {
    ...
    public PlayerCache getOrLoadPlayerCache(UUID uuid, boolean forceLoad) {
        ...
        File file = new File(GlobalMarket.getInstance().getDataFolder() + "/data/", uuid + ".yml");
        FileConfiguration data = YamlConfiguration.loadConfiguration(file);
